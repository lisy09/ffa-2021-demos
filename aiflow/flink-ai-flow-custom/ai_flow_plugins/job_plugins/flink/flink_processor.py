...
class ExecutionContext(json_utils.Jsonable):
    ...
    AINode node configuration parameters(config), StreamExecutionEnvironment, TableEnvironment and StatementSet.
    ...
    def __init__(self,
                 ...
                 config: Dict,
                 execution_env: StreamExecutionEnvironment,
    ...
    @property
    def execution_env(self) -> StreamExecutionEnvironment:
...
def _validate_create_statement(stmt, dataset_meta: DatasetMeta):
    ...
    required_connector = None if dataset_meta.properties is None else dataset_meta.properties.get(
        'connector')
