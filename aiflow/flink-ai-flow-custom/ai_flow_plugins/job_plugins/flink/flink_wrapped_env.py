...
class WrappedBatchTableEnvironment(WrappedTableEnvironment):

    def __init__(self, j_tenv):
        super().__init__(j_tenv)

    @staticmethod
    def create_from(t_env: TableEnvironment) -> 'WrappedBatchTableEnvironment':
...
class WrappedStatementSet(StatementSet):
    ...
    def add_insert(self, target_path_or_descriptor, table, overwrite: bool = False) -> 'StatementSet':
        result = super().add_insert(target_path_or_descriptor, table, overwrite=overwrite)
        self.wrapped_context.need_execute = True
        return result
