...
from ai_flow_plugins.job_plugins.flink.flink_wrapped_env import (
    WrappedBatchTableEnvironment, WrappedStatementSet, WrappedStreamTableEnvironment,
    WrappedTableEnvironment)
...
class FlinkEnv(object):
    """
    FlinkEnv is responsible for creating the objects(StreamExecutionEnvironment, TableEnvironment, StatementSet)
...
class FlinkBatchEnv(FlinkEnv):
    ...
    def create_env(self) -> Tuple[StreamExecutionEnvironment, WrappedTableEnvironment, WrappedStatementSet]:
        ...
        _env_settings = EnvironmentSettings.in_batch_mode()
        _t_env = TableEnvironment.create(_env_settings)
