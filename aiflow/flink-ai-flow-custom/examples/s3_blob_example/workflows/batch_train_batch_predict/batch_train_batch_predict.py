...
from batch_train_batch_predict_executor import (DatasetReader,
                                                DatasetTransformer,
                                                DatasetWriter,
                                                EvaluateDatasetReader,
                                                EvaluateTransformer,
                                                ModelEvaluator, ModelPredictor,
                                                ModelPusher, ModelTrainer,
                                                ModelValidator,
                                                PredictDatasetReader,
                                                PredictTransformer,
                                                ValidateDatasetReader,
                                                ValidateTransformer)

DATASET_URI = os.path.abspath(os.path.join(
    __file__, "../../../..")) + '/dataset_data/mnist_{}.npz'
...
def build_workflow():
    ...
    with af.job_config('push'):
        ...
        af.push_model(model_info=train_model, pushing_model_processor=ModelPusher(
            push_model_artifact_name))
...
def run_workflow() -> WorkflowExecutionInfo:
    # Run workflow
    ...
    workflow_execution = af.workflow_operation.start_new_workflow_execution(
        af.current_workflow_config().workflow_name)
    print(f"execution namespace: {workflow_execution.workflow_info.namespace}")
    print(
        f"execution workflow: {workflow_execution.workflow_info.workflow_name}")
    print(f"execution id: {workflow_execution.workflow_execution_id}")
    return workflow_execution


def watch_running_and_stop_gracefully(workflow_execution: WorkflowExecutionInfo):
    while True:
        try:
            current_execution = af.workflow_operation.get_workflow_execution(
                workflow_execution.workflow_execution_id)
            if current_execution.status == "FINISHED":
                break
            time.sleep(5)
...
if __name__ == '__main__':
    ...
    watch_running_and_stop_gracefully(execution)
